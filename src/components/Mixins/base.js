...
var baseMixin = {
  ...
  methods: {
    ...
    handleDownload() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          axios({
            url: _this.Urls.downloadExcelUrl,
            method: 'post',
            data: {
              ids: _this.selectedRowKeys
            },
      ...
      if (_this.selectedRowKeys.length > 0) {
        ...
        this.$confirm({
          ...
          onOk() {
            _this.localLoading = true
            axios({
              url: _this.Urls.deriveExcelUrl,
              method: 'post',
              data: _this.queryParam.condition,
              responseType: 'blob'
            }).then(res => {
              if (!res) {
                return
    ...
    handleStartTime(date) {
      this.queryParam.condition.startTime = `${date} 00:00:00`
    },
    handleEndTime(date) {
      this.queryParam.condition.endTime = `${date} 23:59:59`
    },
